...
AF.ToastRenderer = {
  render() {
    if (!AF.state.toasts.length) return '';
    ...
    return `
    <div class="af-toast-container">
      ${AF.state.toasts.map(t => `
        <div class="af-toast af-toast-${t.kind}" data-toast-id="${t.id}">
          <div class="af-toast-icon">${icons[t.kind] || icons.info}</div>
          <div class="af-toast-msg">${t.msg}</div>
          <button class="af-toast-close" data-dismiss-toast="${t.id}">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              <line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/>
            </svg>
          </button>
          <div class="af-toast-progress"></div>
        </div>
      `).join('')}
    </div>`;
  },
  ...
  bind() {
    document.querySelectorAll('[data-dismiss-toast]').forEach(el => {
      el.onclick = () => {
        const id = el.dataset.dismissToast;
